...
export async function initPlayPage(){
  ...
  document.getElementById("submitBtn").onclick = async ()=>{
    ...
    const q = questions[idx];
    const evalRes = DISK.evaluateAnswerFromUI(); // exact match sequence
    const time_ms = startMs ? (Date.now()-startMs) : 0;
    const points = evalRes.isCorrect ? 10 : 0;

    const { error } = await supabase.from("game_answers").upsert({
      room_id,
      player_id,
      question_id: q.id,
      answer: evalRes.answer,
    ...
    if(fb){
      fb.textContent = evalRes.isCorrect
        ? "✅ Submitted (Correct)"
        : `✅ Submitted (Wrong)`;
    }
